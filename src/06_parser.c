...
static Tok *peek(Parser *p){ return &p->tv.v[p->pos]; }
static Tok *prev(Parser *p){ return &p->tv.v[p->pos-1]; }
static int match(Parser *p,TokKind k){ if(peek(p)->kind==k){p->pos++;return 1;} return 0; }
static Tok *need(Parser *p,TokKind k,const char *msg){ if(peek(p)->kind!=k){ fprintf(stderr,"parse error at line %d: expected %s\n",peek(p)->line,msg); exit(1);} return &p->tv.v[p->pos++]; }
static int skip_nl(Parser *p){ int n=0; while(match(p,T_NEWLINE)) n++; return n; }
static int name_const(Parser *p,const char *s){ return add_const(p->chunk,stringv(s)); }

static void expr(Parser *p);
static void assign_stmt(Parser *p);
static void from_import_stmt(Parser *p);
static void legacy_statement(Parser *p);
static int is_assignment(Parser *p);
static Function *alloc_function(const char *name,char **params,int arity,Dict *globals,char *dir,int store_globals);
static void compile_block_ast(Parser *p, Stmt **stmts, int count);
static void compile_stmt_ast(Parser *p, Stmt *s);

/* ========================= AST driven statement compiler ========================= */
static void compile_expr_ast(Parser *p, Expr *e){
...
static Function *compile_function_from_ast(Parser *p,const char *name,char **params,int arity,Stmt **body,int body_count,int store_globals){
    ...
    for(int qi=0; qi<p->chunk->count; qi++) if(p->chunk->code[qi]==OP_YIELD) fn->is_generator=1;
    p->chunk=outer;
    return fn;
}
...
static void compile_block_ast(Parser *p, Stmt **stmts, int count){ for(int i=0;i<count;i++) compile_stmt_ast(p,stmts[i]); }
...
static Function *alloc_function(const char *name,char **params,int arity,Dict *globals,char *dir,int store_globals);
static Function *compile_function_from_parser(Parser *p,const char *name,char **params,int arity,int until_dedent,int store_globals);
static void legacy_statement(Parser *p);
...
static void factor(Parser *p){ power(p); while(peek(p)->kind==T_STAR||peek(p)->kind==T_SLASH||peek(p)->kind==T_FLOOR_DIV){ TokKind k=peek(p)->kind; int line=peek(p)->line; p->pos++; power(p); emit_op(p->chunk,k==T_STAR?OP_MUL:OP_DIV,line); } }
...
static void expr(Parser *p){ and_expr(p); while(match(p,T_OR)){ int line=prev(p)->line; emit_arg(p->chunk,OP_JUMP_IF_TRUE_KEEP,0,line); int end=p->chunk->count-1; emit_op(p->chunk,OP_POP,line); and_expr(p); patch(p->chunk,end,p->chunk->count); } }
...
static void from_import_stmt(Parser *p){ Tok *m=need(p,T_NAME,"module name"); int line=m->line; need(p,T_IMPORT,"import"); do{ Tok *n=need(p,T_NAME,"imported name"); const char *alias=n->text; if(match(p,T_AS)){ Tok *a=need(p,T_NAME,"alias"); alias=a->text; } emit_arg(p->chunk,OP_IMPORT,name_const(p,m->text),line); emit_arg(p->chunk,OP_GET_ATTR,name_const(p,n->text),line); emit_arg(p->chunk,OP_STORE,name_const(p,alias),line); } while(match(p,T_COMMA)); need(p,T_NEWLINE,"newline"); }
...
static Function *alloc_function(const char *name,char **params,int arity,Dict *globals,char *dir,int store_globals){ Obj *o=new_obj(O_FUNCTION); o->as.fn.name=xstrdup2(name); o->as.fn.params=params; o->as.fn.arity=arity; o->as.fn.min_arity=arity; o->as.fn.default_count=0; o->as.fn.defaults=NULL; o->as.fn.chunk=chunk_new(); o->as.fn.globals=globals; o->as.fn.closure=NULL; o->as.fn.module_dir=dir?xstrdup2(dir):xstrdup2("."); o->as.fn.store_globals=store_globals; o->as.fn.is_generator=0; o->as.fn.owner=o; return &o->as.fn; }
static Function *clone_function(Function *src, Dict *closure){ Obj *o=new_obj(O_FUNCTION); o->as.fn=*src; o->as.fn.name=xstrdup2(src->name); o->as.fn.closure=closure; o->as.fn.owner=o; return &o->as.fn; }
static Function *compile_function_from_parser(Parser *p,const char *name,char **params,int arity,int until_dedent,int store_globals){
    Chunk *outer=p->chunk; Function *fn=alloc_function(name,params,arity,p->globals,p->module_dir,store_globals); p->chunk=fn->chunk; skip_nl(p); while(peek(p)->kind!=T_EOF && !(until_dedent && peek(p)->kind==T_DEDENT)) legacy_statement(p); if(until_dedent) need(p,T_DEDENT,"dedent"); emit_op(p->chunk,OP_NONE,peek(p)->line); emit_op(p->chunk,OP_RETURN,peek(p)->line); for(int qi=0; qi<p->chunk->count; qi++) if(p->chunk->code[qi]==OP_YIELD) fn->is_generator=1; p->chunk=outer; return fn;
}
static Function *compile_source(const char *src,const char *name,const char *dir,Dict *globals){
