...
static Value run_prepared(Function *fn, Dict *locals, Obj *gen_obj){
    ...
    for(;;){ Op op=(Op)c->code[fr->ip++];
        switch(op){
            ...
            case OP_DEL_ATTR:{ Value namev=c->consts[c->code[fr->ip++]]; char *name=namev.as.obj->as.str.s; Value obj=popv(); if(is_obj(obj,O_INSTANCE)){ Dict *d=obj.as.obj->as.inst.fields; int ix=dict_find(d,name); if(ix<0) raise_named("AttributeError","no such attribute"); for(int j=ix;j<d->count-1;j++){ d->keys[j]=d->keys[j+1]; d->vals[j]=d->vals[j+1]; } d->count--; } else raise_named("AttributeError","cannot delete attribute"); break; } case OP_SET_INDEX:{ Value val=popv(),idx=popv(),obj=popv(); set_index(obj,idx,val); push(val); break; }
