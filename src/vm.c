...
static Value call_value_ex(Value callee, List *pos, Dict *kw){
    ...
    if(is_obj(callee,O_CLASS)){ Class *kl=&callee.as.obj->as.klass;
        ...
        if(class_find(kl,"__init__",&init)){ int n=pos->count; Value *argv=(Value*)xmalloc(sizeof(Value)*(size_t)(n+1)); argv[0]=self; for(int i=0;i<n;i++) argv[i+1]=pos->items[i]; Function *fn=&init.as.obj->as.fn; Dict *locals=dict_clone(fn->closure); bind_arguments(fn,argv,n+1,kw,locals); run_prepared(fn,locals,NULL); free(argv); }
