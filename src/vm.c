...
static Value run_function_ex(Function *fn,int argc,Value *args, Obj *gen_obj){
    ...
    for(;;){ Op op=(Op)c->code[fr->ip++];
        switch(op){
            ...
            case OP_ADD:{ Value b=popv(),a=popv(); push(binary_add(a,b)); break; } case OP_SUB: case OP_MUL: case OP_DIV: case OP_FLOOR_DIV: case OP_POWER:{ Value b=popv(),a=popv(); push(binary_num(a,b,op)); break; } case OP_NEG:{ Value a=popv(); if(!is_number(a)) runtime_error("bad unary -"); if(a.type==V_FLOAT) push(floatv(-a.as.f)); else push(intv(-as_int(a))); break; }
